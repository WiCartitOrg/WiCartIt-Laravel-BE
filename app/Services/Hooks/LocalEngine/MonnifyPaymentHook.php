...
namespace App\Services\Hooks\LocalEngine;

use Illuminate\Http\Request;

trait MonnifyPaymentHook
{
	//inherits all their methods:

	protected function  CallMonnifyPayWithCardService(array $userCardDetails): bool 
	{
		//init:
    ...
    protected function  CallMonnifyPayWithBankService(array $userBankDetails): bool 
	{
        		//init:
		//call our payment hooks that will interact with the API:
		...
		$charge_success = false;
			...
			$card_charge = $stripe?->charges?->create([
  				...
  				'amount' => $userBankDetails['charge_price'],

  				'currency' => strtolower($userBankDetails['cart_purchase_currency']),

  				'source' => [
  					'object' => 'card',
    				'number' => $userBankDetails['buyer_card_number'],
    				'exp_month' => $userBankDetails['buyer_card_exp_month'],
    				'exp_year' => $userBankDetails['buyer_card_exp_year'],
    				'cvc' => $userBankDetails['buyer_card_cvv'],
    			],
  				...
  				/*'payment_method_details' => [
  					'type' => 'card',
  					'card' => [
  						'brand' => strtolower($userBankDetails['buyer_card_type']),
  						/*'number' => $userBankDetails['buyer_card_number'],
    					'exp_month' => $userBankDetails['buyer_card_exp_month'],
    					'exp_year' => $userBankDetails['buyer_card_exp_year'],
    					'cvc' => $userBankDetails['buyer_card_cvv'],
  				...
  				'receipt_email' => $userBankDetails['buyer_email'],

  				'description' => "Payment for Pending Cart:" . $userBankDetails['pending_cart_id']

			], [
  				'idempotency_key' => $userBankDetails['pending_cart_id']//use the pending cart id for ensuring that a single transaction request is not performed twice. 
			]);
